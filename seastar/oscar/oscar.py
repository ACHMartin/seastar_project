...
def add_central_electromagnetic_wavenumber(ds):
    """
    Calculate wavenumber of central electromagnetic frequency and add to dataset
...
def compute_incidence_angle(ds):
    """
    Calculate incidence angle between radar beam and sea surface and add to dataset
    ...
    -------
    ds : OSCAR SAR dataset in netCDF format.
    ds.IncidenceAngle : Incidence angle between radar beam and sea surface (radians)
...
def compute_antenna_azimuth_direction(ds, antenna):
    ...
    -------
    ds : OSCAR SAR dataset in netCDF format.
    ds.OrbitHeadingImage : Aircraft heading of each image pixel (degrees from North)
    ds.AntennaAzimuthImage : Antenna beam azimuth of each image pixel (degrees from North)

    """
    m,n=ds.OrbTimeImage.shape
    head=np.interp(np.reshape(ds.OrbTimeImage,(1,m*n)), ds.GPSTime, ds.OrbitHeading)
    head=np.reshape(head,(m,n))
    ds['OrbitHeadingImage'] = xr.DataArray.copy(ds.OrbTimeImage,data=head)
    ds.OrbitHeadingImage.attrs['long_name']='Heading (degrees from North) of the airfraft for each pixel in the image'
    ds.OrbitHeadingImage.attrs['units']='[degrees]'

    # Assuming antennas pointing to port. If pointing to starboard then change sign
    if antenna == 'fore':
...
def compute_time_lag_Master_Slave(ds, options='from_SAR_time'):
    """
    Computer time lag tau between Master/Slave images and add to dataset
    ...
    if options == 'from_aircraft_velocity':
        if ~'Baseline' in ds.data_vars:
            add_antenna_baseline(ds, baseline=0.2)
