...
from seastar.retrieval.spatial_ambiguity_selection import (
...
def ambiguity_closest_to_truth(lmout, truth, windcurrentratio=10):
    ...
    (mytruth["CurrentU"], mytruth["CurrentV"]) = seastar.utils.tools.currentVelDir2UV(
        truth.CurrentVelocity, truth.CurrentDirection
    )
    mytruth["x"] = xr.concat(
        [
            mytruth["EarthRelativeWindU"],
            mytruth["EarthRelativeWindV"],
            mytruth["CurrentU"],
            mytruth["CurrentV"],
        ],
        "x_variables",
    ...
    err["dist_x_reduce"] = xr.concat(
        [
            ...
            windcurrentratio
            * (err.x.sel(x_variables="c_u") ** 2 + err.x.sel(x_variables="c_v") ** 2),
        ],
        "x_reduce",
    )
    ...
    ind_dict = dict(
        {
            "windcurrent": err.dist_x_reduce.sum(dim="x_reduce").argmin(
                dim="Ambiguities"
            ),
...
def solve_ambiguity(lmout, ambiguity):
    ...
    if ambiguity["name"] == "sort_by_cost":
        ...
        if "truth" not in ambiguity:
            raise Exception(
                "ambiguity['truth'] HAVE to be provided for closest_truth method with"
                "same size as lmout"
            )
        index_dict = ambiguity_closest_to_truth(
            lmout, ambiguity["truth"], windcurrentratio=ambiguity["windcurrentratio"]
        )
